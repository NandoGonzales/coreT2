 ...
 * Path: /admin/Repayment-Tracker/send_ai_message.php
 */
...
try {
    ...
    $stmt = $conn->prepare("
        SELECT 
            ...
            lp.start_date, lp.end_date, lp.status,
            m.full_name as member_name, m.email, m.contact_no,
            ls.schedule_id, ls.due_date, ls.amount_due, ls.balance,
    ...
    if (!$result || $result->num_rows === 0) {
        jsonResponse(404, ['success' => false, 'message' => 'Loan not found or no pending payments']);
    }

    $loanData = $result->fetch_assoc();

    if (empty($loanData['email'])) {
        jsonResponse(400, ['success' => false, 'message' => 'Member does not have an email address on file']);
    }

    $daysUntilDue = (int)$loanData['days_until_due'];
    $messageType  = 'reminder';
    if      ($daysUntilDue >= 7)                      $messageType = '7_days_before';
    elseif  ($daysUntilDue >= 1 && $daysUntilDue < 7) $messageType = '3_days_before';
    elseif  ($daysUntilDue === 0)                      $messageType = 'due_today';
    elseif  ($daysUntilDue < 0)                        $messageType = 'overdue';
    ...
    if ($action !== 'send') {
        jsonResponse(400, ['success' => false, 'message' => 'Invalid action. Use "preview" or "send".']);
    }

    $messageBody = trim($customMessage) !== '' ? $customMessage : $generatedBody;
    if (trim($messageBody) === '') {
        jsonResponse(400, ['success' => false, 'message' => 'Message body is empty.']);
    }
    ...
    foreach ($requiredKeys as $k) {
        if (empty($settings[$k])) throw new Exception("Missing SMTP setting: {$k}");
    }
    ...
    try {
        ...
        // ✅ Log to ai_message_logs (email_notification_logs skipped — column mismatch)
    ...
    jsonResponse(500, ['success' => false, 'message' => $e->getMessage()]);
}