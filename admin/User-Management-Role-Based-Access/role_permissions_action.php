...
try {
    switch ($action) {
        ...
        case 'delete':
            ...
            $stmt = $conn->prepare("
                SELECT rp.module_name, ur.role_name, rp.role_id
                FROM role_permissions rp
                LEFT JOIN user_roles ur ON rp.role_id = ur.role_id
                WHERE rp.perm_id = ?
            ");
            ...
            if ($stmt->execute()) {
                // Log the action
                $roleName = $permission['role_name'] ?: ($permission['role_id'] == 1 ? 'Super Admin' : ($permission['role_id'] == 2 ? 'Admin' : ($permission['role_id'] == 3 ? 'Staff' : 'Unknown')));
                $log_message = "Deleted permission: {$roleName} - {$permission['module_name']}";
