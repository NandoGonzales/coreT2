      ...
      <form id="userForm">
        ...
        <div class="modal-body">
          ...
          <div class="mb-3">
            ...
            <small class="text-muted" id="passwordHelp" style="display:none;">Leave blank to keep current
              password.</small>
          </div>
          <div class="mb-3">
...
<script>
  ...
  document.getElementById('userForm').addEventListener('submit', function (e) {
    ...
    // Adding now creates a user_creation approval request
    const requestData = {
      username: fd.get('username'),
    ...
    const arFd = new FormData();
    arFd.append('action', 'submit_request');
    arFd.append('user_id', '0'); // 0 indicates a new user
    arFd.append('request_type', 'user_creation');
    arFd.append('request_data', JSON.stringify(requestData));
    ...
    fetch('approval_action.php', { method: 'POST', body: arFd })
      ...
      .then(resp => {
        if (resp.status === 'success') {
          Swal.fire('Request Submitted', 'The request to add a new user has been submitted for administrative approval.', 'success');
          userModal.hide();
          loadUsers();
        } else {
          Swal.fire('Error', resp.msg, 'error');
      ...
      .catch(() => Swal.fire('Error', 'Failed to submit user creation request.', 'error'));
  });
